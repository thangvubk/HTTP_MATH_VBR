...
public class ThangBufferBased extends AdaptationLogic {
	...
	public ThangBufferBased(Buffer buffer, Manifest manifest) {
	...
	protected AdaptationDecision decideNextRequest() {
		...
		if (buffer.getBufferSize() < minBuffer) {
			...
			if (currentParams.bitrate > estimatedNextThroughput) {
				...
				nextNSegment = (nextNSegment == maxN) ? nextNSegment : nextNSegment++;
			}
